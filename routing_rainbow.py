...
class RoutingRainbowAgent:
    ...
    def __init__(
            self,
            env: RoutingEnvironmentTSN | JointEnvironmentTSN,
            log_file_id: str
    ):
        """Initialization.
        ...
        # Logging settings
        self.logger = logging.getLogger('routing_rainbow')
        ...
        # Spaces
        obs_dim = env.routing_state_space.shape[0]
        action_dim = env.routing_action_space.n
    ...
    def _plot(step: int, max_steps: int, scores: List[float], mean_scores: List[float], losses: List[float],
        ...
        if SAVE_PLOTS and step == max_steps:
            ...
            plt.savefig(PLOTS_PATH + 'plot_' + date + '_routing_' + str(int(np.mean(scores[-100:]))) + '.png')
    ...
    def train(self, max_steps: int, monitor_training: int = 5000, plotting_interval: int = 20000):
        ...
        try:
            for step in range(1, max_steps + 1):
                ...
                episode_data['Terminal_state'] += info['Terminal_state']
                ...
                if step % monitor_training == 0:
                    ...
                    self.logger.info(f"[I] Elapsed time: {elapsed_time_formatted} | "
                                     ...
                                     f"Reached target: {episode_data['Reached_target']} | "
                                     f"Optimal routes: {episode_data['Optimal_route']} | "
                                     f"Delayed service: {episode_data['Delay_exceeded']} | "
                                     f"Terminal states: {episode_data['Terminal_state']}")
        ...
        if episodes_count > 1000:
            date = datetime.now(tz=timezone(timedelta(hours=2))).strftime('%Y%m%d%H%M')
            model_name = 'model_' + date + '_routing_' + str(int(np.mean(scores[-100:]))) + '.pt'
    ...
    def evaluate(self, max_episodes):
        ...
        self.logger.info(f"[I] Reached target: {episode_data['Reached_target']} | "
                         f"Optimal routes: {episode_data['Optimal_route']} | "
                         f"Delayed service: {episode_data['Delay_exceeded']} | "
                         f"Terminal states: {episode_data['Terminal_state']}")
